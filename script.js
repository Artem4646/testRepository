...
function renderStep() {
    ...
    if (currentMode === 'write' || currentMode === 'choice' || currentMode === 'test') {
        let isWriteMode = currentMode === 'write' || (currentMode === 'test' && Math.random() > 0.5);
...
function checkChoice(btn, user, cor) {
    ...
    if (!isCor) {
        document.querySelectorAll('#mode-container button').forEach(b => {
            if (b.innerText.trim() === cor) b.style.backgroundColor = 'var(--success)';
        });
    }
